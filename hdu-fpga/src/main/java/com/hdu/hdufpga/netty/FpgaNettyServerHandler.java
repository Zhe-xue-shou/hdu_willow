...
//        }
  ...
  private void processLoginReq(ChannelHandlerContext ctx, String ip) {
    CircuitBoardPO cbPO = circuitBoardService.getCircuitBoardByIp(ip);
    String longId;
    if (cbPO != null) {
      longId = cbPO.getLongId();
    } else {
  ...
  private void processOKReq(ChannelHandlerContext ctx, String longId) {
    ...
    CircuitBoardUtil.recordBitToCB(ctx, filepath, longId, count + 1);
  ...
  private CircuitBoardPO initCbObject(String longId, String ip) {
    ...
    circuitBoardPO.setCreateTime(TimeUtil.getNowTime());
  ...
  private String findNewLongIdFromDB() {
    String longId = circuitBoardService.getNextLongId();
    log.info("获取到新的longId为：{}", longId);
    return longId;
  }
}
